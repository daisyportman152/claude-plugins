 ...
 * group support with mention-triggering, and forum topic routing.
 * State lives in ~/.claude/channels/telegram/access.json — managed by
 * the /telegram:access skill.
 *
 * Telegram's Bot API has no history or search. Reply-only tools.
 * Fork: added message_thread_id support for supergroup forum topics.
 */
  ...
  tools: [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          message_thread_id: {
            type: 'string',
            description: 'Topic/forum thread ID for supergroups with Topics enabled. Pass message_thread_id from inbound <channel> meta to reply in the same topic.',
          },
          files: {
            type: 'array',
            items: { type: 'string' },
...
async function transcribeVoice(fileId: string): Promise<string | undefined> {
  ...
  try {
    ...
    if (!res.ok) {
      process.stderr.write(`telegram channel: groq transcription failed: HTTP ${res.status}\n`)
      return undefined
    }
